...
Official authorities and reporting correspondents on the ground continue to monitor developments closely as further communiqués and statements are anticipated in the coming hours.`}resolveContextualNewsImage(e,t,s,a,i=0){if(e&&!e.includes("1x1")&&!e.includes("pixel")&&!e.includes("favicon"))return e;const n=`${t} ${s}`.toLowerCase();return a.region==="india"||a.isPolitics||n.includes("india")||n.includes("delhi")?n.includes("supreme court")||n.includes("high court")||n.includes("judge")||n.includes("judiciary")||n.includes("verdict")||n.includes("bench")||n.includes("bail")||n.includes("cbi")||n.includes("ed")?E.judiciary_law[0]:n.includes("isro")||n.includes("space")||n.includes("satellite")||n.includes("chandrayaan")||n.includes("drdo")||n.includes("missile")?E.defense_isro[i%E.defense_isro.length]:n.includes("rupee")||n.includes("rbi")||n.includes("reserve bank")||n.includes("bank")||n.includes("sensex")||n.includes("nifty")||n.includes("gdp")||n.includes("inflation")||n.includes("finance minister")?E.economy_rbi[i%E.economy_rbi.length]:n.includes("solar")||n.includes("renewable")||n.includes("power")||n.includes("green energy")?E.clean_energy[0]:n.includes("semiconductor")||n.includes("chip")||n.includes("fab")||n.includes("hardware")?E.technology[0]:E.parliament_governance[i%E.parliament_governance.length]:R[i%R.length]}isPoliticalArticle(e,t){const s=`${e} ${t}`.toLowerCase();return["parliament","lok sabha","rajya sabha","bjp","congress","election","elections","minister","cabinet","supreme court","chief minister","prime minister","modi","rahul gandhi","governor","mla","mp","assembly","ordinance","bill","judiciary","constitution","poll","party","yogi","kejriwal","shah","niti aayog","electoral","high court"].some(i=>s.includes(i))}parseDateSafe(e){if(!e)return new Date().toISOString();const t=new Date(e);return isNaN(t.getTime())?new Date().toISOString():t.toISOString()}stripHtml(e){if(!e)return"";const t=document.createElement("DIV");return t.innerHTML=e,t.textContent||t.innerText||""}extractCleanSnippet(e){const t=this.stripHtml(e);if(!t)return"Read the complete verified story on the original publisher broadcast.";const s=t.replace(/\s+/g," ").trim();return s.length>260?s.substring(0,257)+"...":s}extractImageFromXml(e,t){const s=Array.from(e.querySelectorAll("enclosure"));for(const n of s){const o=n.getAttribute("type")||"",r=n.getAttribute("url");if(r&&(o.startsWith("image")||r.match(/\.(jpeg|jpg|png|webp)/i)))return r}const a=Array.from(e.getElementsByTagNameNS("*","content")).concat(Array.from(e.getElementsByTagNameNS("*","thumbnail")));for(const n of a){const o=n.getAttribute("url");if(o&&!o.includes("1x1")&&!o.includes("analytics")&&!o.includes("logo"))return o}const i=e.querySelector("storyimage, thumb, image > url, fullimage");if(i&&i.textContent){const n=i.textContent.trim();if(n.startsWith("http"))return n}return this.extractImageFromHtml(t)}extractImageFromHtml(e){if(e){const t=e.matchAll(/<img[^>]+(?:src|data-src|data-original)=["']([^"']+)["']/gi);for(const s of t){const a=s[1];if(a&&!a.includes("feedburner")&&!a.includes("analytics")&&!a.includes("1x1")&&!a.includes("pixel")&&!a.includes("badge")&&!a.includes("button")&&!a.includes("icon"))return a}}return null}generateKeyPoints(e,t,s){return[`Primary Development: ${e}`,`Core Briefing: ${t.length>30?t.substring(0,140)+"...":"Verified wire bulletin from "+s.name+"."}`,`Verified Media Wire: Real-time update monitored on ${s.name} editorial network.`]}async fetchArticleOgImage(e){if(!e||!e.startsWith("http"))return null;const t=m.getArticleImage(e);if(t)return t;try{const s=`/api/proxy-rss?url=${encodeURIComponent(e)}&extract=image`,a=await fetch(s,{signal:AbortSignal.timeout(4e3)});if(a.ok){const i=await a.json();if(i&&i.success&&i.image)return m.setArticleImage(e,i.image),i.image}}catch{}try{const s=`https://api.allorigins.win/get?url=${encodeURIComponent(e)}`,a=await fetch(s,{signal:AbortSignal.timeout(4e3)});if(a.ok){const n=(await a.json()).contents||"",o=n.match(/<meta[^>]+(?:property|name)=["'](?:og:image|og:image:url|twitter:image|twitter:image:src)["'][^>]+content=["']([^"']+)["']/i)||n.match(/<meta[^>]+content=["']([^"']+)["'][^>]+(?:property|name)=["'](?:og:image|og:image:url|twitter:image|twitter:image:src)["']/i);if(o&&o[1]){let r=o[1].trim().replace(/&amp;/g,"&");if(r.startsWith("//")&&(r="https:"+r),r.startsWith("http"))return m.setArticleImage(e,r),r}}}catch{}return null}async enrichSourceArticlesWithOgImages(e){if(!e||e.length===0)return e;const t=e.filter(s=>s.hasPlaceholderImage&&s.link);return t.length===0||await Promise.allSettled(t.slice(0,10).map(async s=>{try{const a=await this.fetchArticleOgImage(s.link);a&&(s.image=a,s.hasPlaceholderImage=!1)}catch{}})),e}async enrichArticlesWithImages(e,t){if(!e||e.length===0)return;const s=e.filter(i=>i.hasPlaceholderImage&&i.link);if(s.length===0)return;const a=3;for(let i=0;i<s.length;i+=a){const n=s.slice(i,i+a);await Promise.allSettled(n.map(async o=>{try{const r=await this.fetchArticleOgImage(o.link);r&&r!==o.image&&(o.image=r,o.hasPlaceholderImage=!1,typeof t=="function"&&t(o))}catch{}}))}}}const U=new V;class K{constructor(){this.synth=window.speechSynthesis,this.currentUtterance=null,this.isPlaying=!1,this.isPaused=!1,this.currentArticleId=null,this.callbacks=[],this.voices=[],this.synth&&(this.loadVoices(),speechSynthesis.onvoiceschanged!==void 0&&(speechSynthesis.onvoiceschanged=()=>this.loadVoices()))}loadVoices(){this.voices=this.synth.getVoices()}getBestVoice(){return this.voices.length||this.loadVoices(),this.voices.find(t=>t.lang.startsWith("en")&&(t.name.includes("Natural")||t.name.includes("Google")||t.name.includes("Premium")))||this.voices.find(t=>t.lang.startsWith("en"))||this.voices[0]}speakArticle(e,t){if(!this.synth){console.warn("Speech synthesis not supported in this browser.");return}if(this.isPlaying&&this.currentArticleId===e.id){this.isPaused?this.resume():this.pause();return}this.stop();const s=e.title?e.title.replace(/<[^>]*>?/gm,""):"",a=e.snippet?e.snippet.replace(/<[^>]*>?/gm,""):"",i=e.content?e.content.replace(/<[^>]*>?/gm,""):"",n=`Story from ${e.source}. Headline: ${s}. Summary: ${a||i}`;this.currentUtterance=new SpeechSynthesisUtterance(n),this.currentUtterance.voice=this.getBestVoice(),this.currentUtterance.rate=1,this.currentUtterance.pitch=1,this.currentArticleId=e.id,this.isPlaying=!0,this.isPaused=!1,this.currentUtterance.onstart=()=>{this.notifyStateChange("playing",e)},this.currentUtterance.onend=()=>{this.isPlaying=!1,this.isPaused=!1,this.currentArticleId=null,this.notifyStateChange("ended",e),t&&t()},this.currentUtterance.onerror=o=>{console.warn("Speech synthesis error",o),this.isPlaying=!1,this.isPaused=!1,this.currentArticleId=null,this.notifyStateChange("ended",e)},this.synth.speak(this.currentUtterance)}pause(){this.synth&&this.isPlaying&&!this.isPaused&&(this.synth.pause(),this.isPaused=!0,this.notifyStateChange("paused"))}resume(){this.synth&&this.isPlaying&&this.isPaused&&(this.synth.resume(),this.isPaused=!1,this.notifyStateChange("playing"))}stop(){this.synth&&(this.synth.cancel(),this.isPlaying=!1,this.isPaused=!1,this.currentArticleId=null,this.notifyStateChange("stopped"))}subscribe(e){return this.callbacks.push(e),()=>{this.callbacks=this.callbacks.filter(t=>t!==e)}}notifyStateChange(e,t=null){this.callbacks.forEach(s=>s({state:e,article:t,articleId:this.currentArticleId,isPlaying:this.isPlaying,isPaused:this.isPaused}))}}const T=new K,J="modulepreload",Y=function(y){return"/"+y},O={},M=function(e,t,s){let a=Promise.resolve();if(t&&t.length>0){let n=function(d){return Promise.all(d.map(c=>Promise.resolve(c).then(h=>({status:"fulfilled",value:h}),h=>({status:"rejected",reason:h}))))};document.getElementsByTagName("link");const o=document.querySelector("meta[property=csp-nonce]"),r=(o==null?void 0:o.nonce)||(o==null?void 0:o.getAttribute("nonce"));a=n(t.map(d=>{if(d=Y(d),d in O)return;O[d]=!0;const c=d.endsWith(".css"),h=c?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${d}"]${h}`))return;const u=document.createElement("link");if(u.rel=c?"stylesheet":J,c||(u.as="script"),u.crossOrigin="",u.href=d,r&&u.setAttribute("nonce",r),document.head.appendChild(u),c)return new Promise((b,v)=>{u.addEventListener("load",b),u.addEventListener("error",()=>v(new Error(`Unable to preload CSS for ${d}`)))})}))}function i(n){const o=new Event("vite:preloadError",{cancelable:!0});if(o.payload=n,window.dispatchEvent(o),!o.defaultPrevented)throw n}return a.then(n=>{for(const o of n||[])o.status==="rejected"&&i(o.reason);return e().catch(i)})},L={apiKey:"AIzaSyDemoPlaceholderKeyForWhatsGoingOn",authDomain:"whats-going-on-news.firebaseapp.com",projectId:"whats-going-on-news",storageBucket:"whats-going-on-news.appspot.com",messagingSenderId:"123456789012",appId:"1:123456789012:web:abcdef123456"},k={CURRENT_USER:"wgo_auth_user_v1",REGISTERED_USERS:"wgo_registered_users_v1",BREAKING_ALERT:"wgo_breaking_alert_v1"},H=1440*60*1e3;function N(y){try{return btoa(encodeURIComponent(y+"_wgo_salt_2026"))}catch{return y}}function q(y,e){return N(y)===e}const x={e:"dhananjaysaini2006@gmail.com",h:N("242622")};class Q{constructor(){this.currentUser=null,this.authListeners=[],this.isFirebaseLive=!1,this.firebaseApp=null,this.firebaseAuth=null,this.init()}async init(){this.loadSavedSession(),this.ensureAdminExists();try{if(L.apiKey&&!L.apiKey.includes("Placeholder")){const{initializeApp:e}=await M(async()=>{const{initializeApp:s}=await import("./index.esm-D_m3LDGF.js");return{initializeApp:s}},__vite__mapDeps([0,1])),{getAuth:t}=await M(async()=>{const{getAuth:s}=await import("./index.esm-8z07djW7.js");return{getAuth:s}},__vite__mapDeps([2,1]));this.firebaseApp=e(L),this.firebaseAuth=t(this.firebaseApp),this.isFirebaseLive=!0,console.info("Connected to live Firebase Cloud project.")}}catch{console.info("Using local Firebase Authentication adapter.")}}loadSavedSession(){try{const e=localStorage.getItem(k.CURRENT_USER);e&&(this.currentUser=JSON.parse(e))}catch{this.currentUser=null}}ensureAdminExists(){const e=this.getAllUsers();if(!e.some(s=>s.email.toLowerCase()===x.e.toLowerCase())){e.unshift({uid:"admin-ds-wgo",email:x.e,displayName:"Dhananjay Saini",role:"admin",passwordHash:x.h,createdAt:"2026-09-02T00:00:00.000Z"});try{localStorage.setItem(k.REGISTERED_USERS,JSON.stringify(e))}catch{}}}getAllUsers(){try{const e=localStorage.getItem(k.REGISTERED_USERS);return e?JSON.parse(e):[]}catch{return[]}}getCurrentUser(){return this.currentUser}isAdmin(){return this.currentUser&&(this.currentUser.role==="admin"||this.currentUser.email&&this.currentUser.email.toLowerCase()===x.e.toLowerCase())}onAuthStateChanged(e){return this.authListeners.push(e),e(this.currentUser),()=>{this.authListeners=this.authListeners.filter(t=>t!==e)}}notifyAuthChanged(){this.authListeners.forEach(e=>{try{e(this.currentUser)}catch(t){console.error("Error in auth listener:",t)}})}async signIn(e,t){const s=(e||"").trim().toLowerCase(),a=(t||"").trim();if(!s||!a)throw new Error("Please provide both email and password.");if(!s.includes("@"))throw new Error("Please enter a valid email address.");if(s===x.e.toLowerCase()){if(q(a,x.h))return this.currentUser={uid:"admin-ds-wgo",email:x.e,displayName:"Dhananjay Saini",role:"admin",photoURL:null},localStorage.setItem(k.CURRENT_USER,JSON.stringify(this.currentUser)),this.notifyAuthChanged(),this.currentUser;throw new Error("Incorrect password. Please try again.")}const n=this.getAllUsers().find(r=>r.email.toLowerCase()===s);if(!n)throw new Error("No account found with this email. Please sign up first.");if(!(n.passwordHash?q(a,n.passwordHash):n.passwordHash===a))throw new Error("Invalid password. Please try again.");return this.currentUser={uid:n.uid,email:n.email,displayName:n.displayName,role:n.role||"reader",photoURL:n.photoURL||null},localStorage.setItem(k.CURRENT_USER,JSON.stringify(this.currentUser)),this.notifyAuthChanged(),this.currentUser}async signUp(e,t,s=""){const a=(e||"").trim().toLowerCase(),i=(t||"").trim(),n=(s||"").trim()||a.split("@")[0];if(!a||!i)throw new Error("Please provide email and password.");if(!a.includes("@"))throw new Error("Please enter a valid email address.");if(i.length<8)throw new Error("Password must be at least 8 characters long.");const o=this.getAllUsers();if(o.some(d=>d.email.toLowerCase()===a))throw new Error("An account with this email already exists. Please sign in.");const r={uid:"user-"+Date.now(),email:a,displayName:n,role:"reader",passwordHash:N(i),createdAt:new Date().toISOString()};o.push(r);try{localStorage.setItem(k.REGISTERED_USERS,JSON.stringify(o))}catch(d){console.warn("Failed to save user",d)}return this.currentUser={uid:r.uid,email:r.email,displayName:r.displayName,role:r.role,photoURL:null},localStorage.setItem(k.CURRENT_USER,JSON.stringify(this.currentUser)),this.notifyAuthChanged(),this.currentUser}async signOut(){this.currentUser=null,localStorage.removeItem(k.CURRENT_USER),this.notifyAuthChanged()}getBreakingAlert(){try{const e=localStorage.getItem(k.BREAKING_ALERT);if(!e)return null;const t=JSON.parse(e);return t.timestamp&&Date.now()-new Date(t.timestamp).getTime()>H?(localStorage.removeItem(k.BREAKING_ALERT),null):t}catch{return null}}setBreakingAlert(e,t="high"){if(!this.isAdmin())throw new Error("Unauthorized. Only Admins can broadcast alerts.");const s={id:"alert-"+Date.now(),text:e,urgency:t,timestamp:new Date().toISOString(),author:this.currentUser.displayName,expiresAt:new Date(Date.now()+H).toISOString()};return localStorage.setItem(k.BREAKING_ALERT,JSON.stringify(s)),s}clearBreakingAlert(){if(!this.isAdmin())throw new Error("Unauthorized.");localStorage.removeItem(k.BREAKING_ALERT)}}const f=new Q;class X{constructor(e,t,s,a,i,n,o){this.onRefreshCallback=e,this.onSearchCallback=t,this.onThemeToggleCallback=s,this.onIntervalChangeCallback=a,this.onPublishClickCallback=i,this.onAuthClickCallback=n,this.onAdminClickCallback=o,this.remainingSeconds=m.getRefreshInterval(),this.totalSeconds=this.remainingSeconds,this.countdownTimer=null,this.clockTimer=null}init(){this.renderHeader(),this.startLiveClock(),this.startCountdown(),this.attachEventListeners(),this.updateBookmarkBadge(),this.initMobileDrawer()}renderHeader(){const e=document.getElementById("top-utility-bar"),t=document.getElementById("main-header");e&&(e.innerHTML=`
      ...
      `).join(""),(d=i.querySelector("#btn-mobile-drawer-publish-cta"))==null||d.addEventListener("click",()=>{o(),this.onPublishClickCallback&&this.onPublishClickCallback()}),i.querySelectorAll(".mobile-drawer-cat-btn").forEach(c=>{c.addEventListener("click",()=>{const h=document.querySelector(`.cat-tab[data-category="${c.dataset.cat}"]`);h&&h.click(),i.querySelectorAll(".mobile-drawer-cat-btn").forEach(u=>u.classList.remove("active")),c.classList.add("active"),o(),window.scrollTo({top:0,behavior:"smooth"})})}))}startLiveClock(){const e=()=>{const t=new Date,s=document.getElementById("current-clock-time"),a=document.getElementById("current-clock-date");s&&(s.textContent=t.toLocaleTimeString("en-IN",{hour:"2-digit",minute:"2-digit",second:"2-digit",hour12:!0})),a&&(a.textContent=t.toLocaleDateString("en-US",{weekday:"short",month:"short",day:"numeric",year:"numeric"}))};e(),this.clockTimer=setInterval(e,1e3)}startCountdown(){this.countdownTimer&&clearInterval(this.countdownTimer),this.totalSeconds=m.getRefreshInterval(),this.remainingSeconds=this.totalSeconds,this.nextRefreshTime=Date.now()+this.totalSeconds*1e3;const e=document.getElementById("countdown-circle-bar"),t=document.getElementById("countdown-text"),s=2*Math.PI*7;e&&(e.style.strokeDasharray=`${s}`);const a=()=>{const i=Date.now(),n=this.nextRefreshTime-i,o=Math.max(0,Math.ceil(n/1e3));this.remainingSeconds=o;const r=Math.floor(this.remainingSeconds/60),d=this.remainingSeconds%60;if(t&&(t.textContent=`${r.toString().padStart(2,"0")}:${d.toString().padStart(2,"0")}`),e){const c=Math.min(1,Math.max(0,this.remainingSeconds/this.totalSeconds)),h=s*(1-c);e.style.strokeDashoffset=`${h}`}o<=0&&(this.resetCountdown(),t&&(t.textContent="Syncing..."),this.onRefreshCallback&&this.onRefreshCallback(!0))};a(),this.countdownTimer=setInterval(a,1e3),this.visibilityListenerAttached||(this.visibilityListenerAttached=!0,document.addEventListener("visibilitychange",()=>{document.visibilityState==="visible"&&a()}))}resetCountdown(){this.totalSeconds=m.getRefreshInterval(),this.remainingSeconds=this.totalSeconds,this.nextRefreshTime=Date.now()+this.totalSeconds*1e3}updateLastSyncTime(e=new Date){const t=document.getElementById("last-sync-time");if(t){const s=e.toLocaleTimeString("en-IN",{hour:"2-digit",minute:"2-digit",second:"2-digit",hour12:!0});t.textContent=`Synced: ${s}`}}updateBookmarkBadge(){const e=document.getElementById("header-bookmark-badge"),t=m.getBookmarks().length;e&&(e.textContent=t,e.style.display=t>0?"flex":"none")}updateUserAuthState(e){var h,u,b,v,I;const t=document.getElementById("header-auth-container"),s=document.getElementById("topbar-auth-pill"),a=document.getElementById("btn-open-publish-modal"),i=e&&(e.role==="admin"||e.email&&e.email.toLowerCase()==="dhananjaysaini2006@gmail.com");a&&(i?(a.classList.add("is-admin"),a.style.setProperty("display","inline-flex","important")):(a.classList.remove("is-admin"),a.style.setProperty("display","none","important")));const n=document.getElementById("topbar-btn-publish");n&&n.style.setProperty("display",i?"inline-flex":"none","important");const o=document.getElementById("mobile-drawer-publish-container");o&&o.style.setProperty("display",i?"block":"none","important");const r=document.getElementById("nav-btn-publish");r&&(i?(r.classList.add("is-admin"),r.style.setProperty("display","inline-flex","important")):(r.classList.remove("is-admin"),r.style.setProperty("display","none","important")));const d=document.getElementById("footer-publish-item");if(d&&d.style.setProperty("display",i?"block":"none","important"),e){if(s&&(s.innerHTML=`
        ...
        `;const g=t.querySelector("#btn-header-admin-panel");g&&g.addEventListener("click",()=>{this.onAdminClickCallback&&this.onAdminClickCallback()});const p=t.querySelector("#btn-header-signout");p&&p.addEventListener("click",async()=>{await f.signOut()})}}else{if(s){s.innerHTML=`
        ...
        `;const g=s.querySelector("#btn-topbar-signin");g&&g.addEventListener("click",S=>{S.preventDefault(),this.onAuthClickCallback&&this.onAuthClickCallback("signin")});const p=s.querySelector("#btn-topbar-signup");p&&p.addEventListener("click",S=>{S.preventDefault(),this.onAuthClickCallback&&this.onAuthClickCallback("signup")})}if(t){t.innerHTML=`
    ...
    `,t.querySelectorAll(".ticker-item").forEach(o=>{o.addEventListener("click",()=>{const r=o.getAttribute("data-id"),d=this.articles.find(c=>c.id===r)||this.articles[0];this.onArticleClickCallback&&d&&this.onArticleClickCallback(d)})})}}class ee{constructor(e,t){this.onArticleClickCallback=e,this.onBookmarkToggleCallback=t,this.viewMode=m.getViewMode()}setViewMode(e){this.viewMode=e,m.setViewMode(e)}formatTimeAgo(e){try{const t=new Date(e),a=new Date-t,i=Math.floor(a/(1e3*60));if(i<1)return"Just now";if(i<60)return`${i}m ago`;const n=Math.floor(i/60);return n<24?`${n}h ago`:`${Math.floor(n/24)}d ago`}catch{return"Recently"}}render(e,t={}){const s=document.getElementById("main-news-feed");if(!s)return;const{category:a="all",searchQuery:i="",sourceFilter:n="all"}=t;if(!e||e.length===0){const v=t.category==="saved";if(t.category===l.PUBLISHED){s.innerHTML=`
        ...
        `;return}s.innerHTML=v?`<div class="empty-state-premium">
          ...
          </div>`;return}if(a===l.ALL&&!i&&n==="all"){this.renderHomeAllNews(e,s);return}const o=!i&&e.length>0,r=o?e[0]:null,d=o?e.slice(1):e,c=r?this.buildHeroCardHtml(r):"",h=this.buildArticlesGridHtml(d),u=a===l.BUSINESS&&!i?this.buildMarketIndicesDashboardHtml():"";let b="Latest Stories";i?b=`Search Results: "${i}"`:a===l.INDIA_POLITICS?b="🏛️ Indian Politics & National Governance":a===l.INDIA?b="🇮🇳 Indian Media Wires":a===l.WORLD?b="🌍 Global News Wires & International":a===l.BUSINESS?b="📊 Global & Indian Business Wire":a===l.GOOGLE_NEWS?b="🌐 Google News Live Wire":a===l.SAVED?b="🔖 Saved Bookmarks":a===l.PUBLISHED?b="✍️ Community & Published Articles":b=`${a.toUpperCase()} Wire`,s.innerHTML=`
      ${u}
    ...
    `}renderHomeAllNews(e,t){const s=e[0],a=e.slice(1,5),i=e.filter(h=>h.region==="india"||h.category===l.INDIA||h.category===l.INDIA_POLITICS),n=e.filter(h=>h.region==="global"||h.category===l.WORLD||h.category===l.GOOGLE_NEWS),o=e.filter(h=>h.category===l.BUSINESS),r=e.filter(h=>h.category===l.TECH||h.category===l.SCIENCE),d=e.slice(5,13),c=s.region==="india"?"NEW DELHI —":s.region==="global"?"GLOBAL WIRE —":"DISPATCH —";t.innerHTML=`
    ...
    `}buildArticlesGridHtml(e){return this.viewMode==="grid"?`
      ...
      `}attachCardEventListeners(e){const t=document.getElementById("main-news-feed");t&&t.addEventListener("click",s=>{const a=s.target.closest("[data-action]"),i=s.target.closest("[data-id]");if(!i)return;const n=i.getAttribute("data-id"),o=e.find(r=>r.id===n);if(o)if(a){const r=a.getAttribute("data-action");if(r==="open-article")this.onArticleClickCallback&&this.onArticleClickCallback(o);else if(r==="bookmark"){const d=m.toggleBookmark(o);a.classList.toggle("bookmarked",d);const c=a.querySelector("svg");c&&c.setAttribute("fill",d?"currentColor":"none"),this.onBookmarkToggleCallback&&this.onBookmarkToggleCallback(o,d)}else r==="speak-article"?T.speakArticle(o):r==="share"&&(navigator.share?navigator.share({title:o.title,text:o.snippet,url:o.link||window.location.href}).catch(()=>{}):(navigator.clipboard.writeText(o.link||window.location.href),alert("Article link copied to clipboard!")))}else s.target.closest("button")||this.onArticleClickCallback&&this.onArticleClickCallback(o)})}}class te{constructor(){this.activeTopic="all",this.currentAffairsList=[...F]}render(){const e=document.getElementById("main-news-feed");if(!e)return;const t=this.activeTopic==="all"?this.currentAffairsList:this.currentAffairsList.filter(i=>i.category===this.activeTopic),a=new Date().toLocaleDateString("en-US",{weekday:"long",month:"long",day:"numeric",year:"numeric"});e.innerHTML=`
...
`).map(u=>u.trim()).filter(Boolean):["Comprehensive on-the-ground intelligence and editorial assessment.","Analysis corroborated by verified primary sources and statutory records.","Implications reviewed by What's Going On editorial desk."],h={id:this.editingArticleId||"user-art-"+Date.now(),title:e,source:t,sourceId:"user-published",category:a,region:a===l.WORLD?"global":"india",isPolitics:a===l.INDIA_POLITICS||a===l.INDIA,isUserPublished:!0,link:"#user-story-"+Date.now(),image:n,pubDate:new Date().toISOString(),snippet:`${s} ${o}`,content:r,keyPoints:c,readTime:i};m.saveCustomArticle(h),this.close(),this.modalEl.querySelector("#publish-studio-form").reset(),this.onArticlePublishedCallback&&this.onArticlePublishedCallback(h)}}class ae{constructor(e){this.onAuthSuccessCallback=e,this.modalEl=null,this.isOpen=!1,this.currentTab="signin"}init(){this.injectModalHtml(),this.attachEventListeners()}injectModalHtml(){const e=document.getElementById("auth-modal-overlay");e&&e.remove();const t=document.createElement("div");t.id="auth-modal-overlay",t.className="auth-modal-overlay",t.setAttribute("role","dialog"),t.setAttribute("aria-modal","true"),t.setAttribute("aria-hidden","true"),t.innerHTML=`
    ...
    `}}class oe{constructor(){this.articles=[],this.currentCategory=l.ALL,this.searchQuery="",this.selectedSource="all",this.isLoading=!1,this.header=null,this.ticker=null,this.newsGrid=null,this.currentAffairs=null,this.modal=null,this.publishModal=null,this.authModal=null,this.adminPanel=null}async init(){this.applyTheme(m.getTheme()),this.initComponents(),this.renderCategoryNavigation();const e=window.location.hash.replace("#","");e&&Object.values(l).includes(e)&&(this.currentCategory=e,document.querySelectorAll(".cat-tab").forEach(s=>{s.classList.toggle("active",s.getAttribute("data-category")===e)})),await this.fetchNews(!1),this.initBackToTop()}applyTheme(e){document.documentElement.setAttribute("data-theme",e),m.setTheme(e);const t=document.getElementById("theme-icon-sun");t&&(e==="light"?t.innerHTML='<path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"></path>':t.innerHTML=`
          <circle cx="12" cy="12" r="5"></circle>
          <line x1="12" y1="1" x2="12" y2="3"></line><line x1="12" y1="21" x2="12" y2="23"></line>
          <line x1="4.22" y1="4.22" x2="5.64" y2="5.64"></line><line x1="18.36" y1="18.36" x2="19.78" y2="19.78"></line>
          <line x1="1" y1="12" x2="3" y2="12"></line><line x1="21" y1="12" x2="23" y2="12"></line>
          <line x1="4.22" y1="19.78" x2="5.64" y2="18.36"></line><line x1="18.36" y1="5.64" x2="19.78" y2="4.22"></line>
        `)}toggleTheme(){const t=m.getTheme()==="dark"?"light":"dark";this.applyTheme(t)}initComponents(){this.authModal=new ae(i=>{this.showToast(`✨ Welcome to What's Going On, ${i.displayName||i.email}!`,"success")}),this.authModal.init(),this.adminPanel=new ne(()=>{this.renderActiveView(),this.showToast("Article moderation updated.","info")},i=>{this.handleBreakingAlert(i)}),this.adminPanel.init(),this.publishModal=new se(i=>{this.articles=[i,...this.articles.filter(n=>n.id!==i.id)],this.ticker.render(this.articles),this.renderActiveView(),window.scrollTo({top:0,behavior:"smooth"}),this.showToast("🎉 Your article has been published live to What's Going On!","success")}),this.publishModal.init(),this.header=new X(i=>this.fetchNews(i),i=>this.handleSearch(i),()=>this.toggleTheme(),i=>this.handleIntervalChange(i),()=>this.publishModal.open(),i=>this.authModal.open(i),()=>this.adminPanel.open(this.articles)),this.header.init(),f.onAuthStateChanged(i=>{this.header.updateUserAuthState(i);const n=f.isAdmin(),o=document.getElementById("nav-btn-publish");o&&(o.style.display=n?"inline-flex":"none");const r=document.getElementById("footer-publish-item");r&&(r.style.display=n?"block":"none")});const e=f.getBreakingAlert();e&&this.handleBreakingAlert(e),this.modal=new ie((i,n)=>{this.header.updateBookmarkBadge()}),this.ticker=new Z(i=>{this.modal.open(i)}),this.newsGrid=new ee(i=>this.modal.open(i),(i,n)=>{this.header.updateBookmarkBadge(),this.currentCategory===l.SAVED&&this.renderActiveView()}),this.currentAffairs=new te;const t=document.getElementById("btn-bookmarks-view");t&&t.addEventListener("click",()=>{this.setCategory(l.SAVED)});const s=document.getElementById("btn-view-grid"),a=document.getElementById("btn-view-list");s&&a&&(s.addEventListener("click",()=>{s.classList.add("active"),a.classList.remove("active"),this.newsGrid.setViewMode("grid"),this.renderActiveView()}),a.addEventListener("click",()=>{a.classList.add("active"),s.classList.remove("active"),this.newsGrid.setViewMode("compact"),this.renderActiveView()}))}renderCategoryNavigation(){const e=document.getElementById("category-tabs-nav"),t=document.getElementById("select-source-filter"),s=[{id:l.ALL,label:"⚡ All News (Front Page)"},{id:l.INDIA_POLITICS,label:"🏛️ Indian Politics & Governance"},{id:l.INDIA,label:"🇮🇳 India Media Wires"},{id:l.WORLD,label:"🌍 Global News Wires"},{id:l.BUSINESS,label:"📊 Market Indices & Economy"},{id:l.GOOGLE_NEWS,label:"🌐 Google News"},{id:l.CURRENT_AFFAIRS,label:"⭐ Current Affairs Hub",highlight:!0},{id:l.TECH,label:"🤖 Tech & AI"},{id:l.SCIENCE,label:"🚀 Science & Defense"},{id:l.PUBLISHED,label:"✍️ Community & Published"},{id:l.SAVED,label:"🔖 Saved Stories"}];if(e){const a=s.map(o=>`
